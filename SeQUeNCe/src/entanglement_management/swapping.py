...
class EntanglementSwappingA(EntanglementProtocol):
    ...
    def start(self) -> None:
        ...
        # if self.own.timeline.swap_exec_count[ord(self.left_protocol.own.name)-ord('a')][ord(self.right_protocol.own.name)-ord('a')]!= 0:
        #     print('-----------------------\nSwap Threshold reached, so exiting\n-----------------------')
        #     print(f'Swap b/w:  {self.left_protocol.own.name} and {self.right_protocol.own.name} and swap_exec_count:{self.own.timeline.swap_exec_count}')
        #     sys.exit()
        # else:
        self.own.timeline.swap_exec_count[ord(self.left_protocol.own.name)-ord('a')][ord(self.right_protocol.own.name)-ord('a')] += 1
        self.own.timeline.swap_exec_count[ord(self.right_protocol.own.name)-ord('a')][ord(self.left_protocol.own.name)-ord('a')] += 1
        ...
        if x_rand < self.success_probability():
            ...
            # print(f'Entanglement swap successful between {self.left_protocol.own.name, self.right_protocol.own.name}')
            self.own.timeline.swap_success_count[ord(self.left_protocol.own.name)-ord('a')][ord(self.right_protocol.own.name)-ord('a')] += 1
            # print('swap_success_count: ', self.own.timeline.swap_success_count)

            if self.left_protocol.own.name == self.own.timeline.src and self.right_protocol.own.name == self.own.timeline.dst or self.right_protocol.own.name == self.own.timeline.src and self.left_protocol.own.name == self.own.timeline.dst:
                self.own.timeline.stop_rules = True
            ...
            # print(f'Entanglement swap failed between {self.own.name, self.right_protocol.own.name}')
            self.own.timeline.swap_failure_count[ord(self.left_protocol.own.name)-ord('a')][ord(self.right_protocol.own.name)-ord('a')] += 1
            # print('swap_failure_count: ', self.own.timeline.swap_failure_count)
            self.own.timeline.hasSwapFailed = True
            # print('-----------------------\nSwap failed so exiting\n-----------------------')
            sys.exit()
...
class EntanglementSwappingB(EntanglementProtocol):
    """Entanglement swapping protocol for end nodes.
    ...
    def received_message(self, src: str, msg: "EntanglementSwappingMessage") -> None:
        ...
        if msg.fidelity > 0 and self.own.timeline.now() < msg.expire_time:
            ...
            # print(f'Start Time: {self.start_time*1e-12} , End Time: {self.end_time*1e-12} , execution time: {(self.end_time - self.start_time)*1e-12}')
            #self.own.timeline.swap_success_count[ord(self.own.name)-ord('a')][ord(msg.remote_node)-ord('a')] += 1
            #print('swap_success_count: ', self.own.timeline.swap_success_count)


            #if self.own.name == 'a' and msg.remote_node == 'c':

        else:
            # print(f'Entanglement swap failed between {self.own.name, self.another.own.name}')
            ##print(f'Time of Entanglement swap failure: {self.own.timeline.now()}')
            self.update_resource_manager(self.memory, "RAW")
            self.end_time = self.own.timeline.now()
            # print(f'Start Time: {self.start_time*1e-12} , End Time: {self.end_time*1e-12} , execution time: {(self.end_time - self.start_time)*1e-12}')
            #self.own.timeline.swap_failure_count[ord(self.own.name)-ord('a')][ord(msg.remote_node)-ord('a')] += 1
            #print('swap_failure_count: ', self.own.timeline.swap_failure_count)
    ...
    def start(self) -> None:
        ...
        """if self.own.timeline.swap_exec_count[ord(self.own.name)-ord('a')][ord(self.another.own.name)-ord('a')]!= 0:
            print('-----------------------\nSwap failed so exiting\n-----------------------')
            sys.exit()
        else:
            self.own.timeline.swap_exec_count[ord(self.own.name)-ord('a')][ord(self.another.own.name)-ord('a')] = 1
            self.own.timeline.swap_exec_count[ord(self.another.own.name)-ord('a')][ord(self.own.name)-ord('a')] = 1"""
