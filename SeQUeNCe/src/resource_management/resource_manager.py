...
class ResourceManager():
    ...
    def update(self, protocol: "EntanglementProtocol", memory: "Memory", state: str) -> None:
        ...
        # if state == 'ENTANGLED':
        #     print(f'To entangled called for the memory index {memo_info.index} at the node {self.owner.name} and the updated state is {memo_info.state} and the peer node is {memo_info.remote_node}')

        if state == 'ENTANGLED' and self.owner.name == 'c' and memo_info.remote_node == 'a':
            self.owner.timeline.swap_success_count[2][0] += 1

        if state == 'ENTANGLED' and self.owner.name == 'a' and memo_info.remote_node == 'c':
            self.owner.timeline.swap_success_count[0][2] += 1

        if not self.owner.timeline.stop_rules:
            ...
            #If the current node is 'c' and the other node is 'a', we initiate the rule manager for 'd'
            if state == 'ENTANGLED' and self.owner.name == 'c' and memo_info.remote_node == 'a':
                memo_info_for_c = self.memory_manager.__getitem__(1)
                for rule in self.rule_manager:
                    memories_info = rule.is_valid(memo_info_for_c)
                    if len(memories_info) > 0:
                        rule.do(memories_info)
                        for info in memories_info:
                            #if state == 'ENTANGLED':
                ...
                memo_info_for_d = self.owner.timeline.topology.nodes['d'].resource_manager.memory_manager.__getitem__(0)
                for rule in self.owner.timeline.topology.nodes['d'].resource_manager.rule_manager:
                    memories_info = rule.is_valid(memo_info_for_d)
                    if len(memories_info) > 0:
                        rule.do(memories_info)
                        for info in memories_info:
                            #if state == 'ENTANGLED':
